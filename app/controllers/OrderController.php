    ...
    public function processOrder() {
        ...
        if ($validator->passes()) {
            ...
            foreach ($prdt_details as $detail) {
                ...
                $str .= 'Prdct Name: '.$product->name . ' Qty = ' . $detail['qty'];
                $order->products()->attach($detail['id'], array('quantity' => $detail['qty']));
            }

            $message = 'A new order from ' . $data['first_name'] . ' ' . $data['last_name'] . ' phone no. ' . $data['phone'] . '. '
                        . $str . ' Total is Ksh ' . $data['total'];

            $sender = new SendSMS();
            $sender->sendText($message, '+254700203130');
        } else {
